...
class MecaRobot:
    ...
    def disconnect(self):
        ...
        if self.ser != None:
            ...
            self.ser.close()
            print_message('Serial Connection closed')
            # self.sock = None
        
        if self.serjnts != None:
    ...
    def connect(self, _port="COM5", _bd=115200):
        ...
        global ROBOT_MOVING
        self.disconnect()
        print_message('Connecting to robot %s:%i' % (_port, _bd))
        ...
        self.serjnts = serial.Serial(port="COM3",
            baudrate=_bd,
            bytesize=serial.EIGHTBITS,
            parity=serial.PARITY_NONE,
            stopbits=serial.STOPBITS_ONE,
        ...
        if self.ser.isOpen():
            print('Serial Opened %s:%i' % (_port, _bd))
        self.CONNECTED = True
        ROBOT_MOVING = False
        ...
        if self.serjnts.isOpen():
            print('Serial Opened %s:%i' % ('COM3', _bd))
    ...
    def recv_str(self):
        ...
        try:
            bdata = b''
            while True:
                chari = self.ser.read(1)
                if chari == b'' or chari == b'\0':
                    break
                bdata += chari
            if bdata == b'':
                return None
    ...
    def Run(self, cmd, values=None, send_ready=True, sync=None):
        ...
        '''
        同步接收,例如执行 CJNT 命令后，代码会调用 Run('GetJoints', sync=True),
        在 MecaRobot 类的 Run 方法中，sync=True 时会等待并接收机器人返回的数据
        '''
...
global ROBOT
global ROBOT_PORT
global ROBOT_BAUDRATE
global ROBOT_MOVING
global ROBOT_MONITOR_LAST_JOINTS

ROBOT = MecaRobot()
ROBOT_PORT = "COM5"      # IP of the robot
ROBOT_BAUDRATE = 115200          # Communication port of the robot
ROBOT_MOVING = False
...
def TestDriver():    
    RunCommand("CONNECT COM5 115200")
    #RunCommand("CJNT")
...
def RunCommand(linecmd):
    ...
    if linecmd == "":
        ...
        ROBOT_PORT = words[1]
        if nwords >= 3 and nvalues >= 1 and values[0] != 115200:
            #ROBOT_BAUDRATE = values[0]
            print("Using default port 115200, not %i" % ROBOT_BAUDRATE)
